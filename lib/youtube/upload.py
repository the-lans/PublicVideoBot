...
# The APP_TOKEN_FILE variable specifies the name of a file that contains
...
def get_authenticated_service(service, version):
    creds = get_creds_saved()
    service = build(service, version, credentials=creds)
    return service


def initialize_upload(youtube, **kwargs):
    ...
    media = MediaFileUpload(kwargs.get('file'), chunksize=-1, resumable=True)

    tags = None
    if kwargs.get('keywords'):
        tags = kwargs.get('keywords').split(',')
        ...
        try:
            ...
            if response is not None:
                if 'id' in response:
                    ...
                    Exception('The upload failed with an unexpected response: %s' % response)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file', help='Video file to upload', default='test.mp4')
    ...
    parser.add_argument('--privacy', choices=VALID_PRIVACY_STATUSES, default='private', help='Video privacy status.')
